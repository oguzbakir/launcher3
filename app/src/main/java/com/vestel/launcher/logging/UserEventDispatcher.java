...
package com.vestel.launcher.logging;
...
import com.vestel.launcher.DropTarget;
import com.vestel.launcher.ItemInfo;
...
public class UserEventDispatcher {
    ...
    public interface LaunchSourceProvider {
         ...
         */
        void fillInLaunchSourceData(View v, ItemInfo info, LauncherLogProto.Target target, LauncherLogProto.Target targetParent);
    }
    ...
    protected LauncherLogProto.LauncherEvent createLauncherEvent(View v, Intent intent) {
        LauncherLogProto.LauncherEvent event = LoggerUtils.initLauncherEvent(
                LauncherLogProto.Action.TOUCH, v, LauncherLogProto.Target.CONTAINER);
    ...
    public void logAppLaunch(View v, Intent intent) {
        LauncherLogProto.LauncherEvent ev = createLauncherEvent(v, intent);
        if (ev == null) {
            return;
        }
    ...
    public void logActionOnItem(int action, int itemType) {
        LauncherLogProto.LauncherEvent event = LoggerUtils.initLauncherEvent(LauncherLogProto.Action.TOUCH, LauncherLogProto.Target.ITEM);
    ...
    public void logActionOnControl(int action, int controlType) {
        LauncherLogProto.LauncherEvent event = LoggerUtils.initLauncherEvent(LauncherLogProto.Action.TOUCH, LauncherLogProto.Target.CONTROL);
        event.action.touch = action;
    ...
    public void logActionOnContainer(int action, int dir, int containerType) {
        LauncherLogProto.LauncherEvent event = LoggerUtils.initLauncherEvent(LauncherLogProto.Action.TOUCH, LauncherLogProto.Target.CONTAINER);
        event.action.touch = action;
    ...
    public void logDeepShortcutsOpen(View icon) {
        LauncherLogProto.LauncherEvent event = LoggerUtils.initLauncherEvent(
                LauncherLogProto.Action.TOUCH, icon, LauncherLogProto.Target.CONTAINER);
        ...
        event.action.touch = LauncherLogProto.Action.LONGPRESS;
        dispatchUserEvent(event, null);
    }
    ...
    public void logDragNDrop(DropTarget.DragObject dragObj, View dropTargetAsView) {
        LauncherLogProto.LauncherEvent event = LoggerUtils.initLauncherEvent(LauncherLogProto.Action.TOUCH,
                dragObj.dragView,
                dragObj.originalDragInfo,
                LauncherLogProto.Target.CONTAINER,
                dropTargetAsView);
        event.action.touch = LauncherLogProto.Action.DRAGDROP;
